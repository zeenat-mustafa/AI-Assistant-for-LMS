...
def build_rubric_prompt(cells: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "- Determine which sections genuinely require student work by reading the markdown and "
        "code cells together as one continuous flow, in order — never scan cells independently or "
        "pattern-match a fixed list of keywords. Real assignments mark completion-required work in "
        "many different ways; treat the following as illustrative examples of the KIND of thing to "
        "notice, not an exhaustive or literal checklist, since a real notebook may use a form not "
        "listed here at all: (1) a markdown question whose answer belongs in a later response cell "
        "(comment or print); (2) a data literal, e.g. a list or dict, with an inline comment blank "
        "inside it; (3) a multi-step comment-only scaffold with zero code structure at all, not even "
        "'pass'; (4) a real partial implementation with the hard part stubbed as 'pass'; (5) a bare "
        "function stub whose full spec is given only in markdown above it; (6) a constructor or "
        "function call with empty arguments and only a comment describing what goes there; (7) a "
        "sentinel placeholder value explicitly flagged for replacement, e.g. None or {} marked "
        "'# replace'; (8) a literal blank (underscores or similar) inside an otherwise-complete "
        "function call; and a fully open-ended cell with only high-level instructions and no "
        "scaffolding at all. Also include a question in one markdown cell whose answer belongs in a "
        "later response cell, and informal comments asking for answer/code regardless of wording.\n"
        ...
        "checks such as 'runs without error'. Write this as ONE consolidated criterion covering the "
        "whole pre-written/scaffolding portion of the notebook, not several small separate criteria "
        "for individual pre-written cells — grading will assess it proportionally, scoring it "
        "against the fraction of that scaffolding a student genuinely re-executes themselves (a "
        "pre-written cell's execution metadata identical to this unsolved file does not count as "
        "the student's own execution), rather than all-or-nothing, so a single combined criterion is "
        "both fairer and impossible to game by splitting it into many small independently-awardable "
        "pieces.\n"
