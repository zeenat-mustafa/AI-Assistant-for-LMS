...
export function CitationList({ citations }: { citations: Citation[] }) {
  ...
  const hasNotebook = citations.some((c) => c.source_type === "notebook");

  return (
    <div className="mt-3 border-t border-slate-200 pt-2" data-testid="citation-list">
      <p className="text-xs font-medium text-slate-600">Course material given to the assistant</p>
      ...
      {hasNotebook ? (
        <p className="mt-1 text-[11px] text-slate-500">
          Notebook cell numbers are 0-indexed (the first cell is Cell 0).
        </p>
      ) : null}
    </div>
  );
...
function locatorParts(citation: Citation): string[] {
  ...
  if (citation.source_type === "lecture") {
    ...
    if (citation.source) parts.push(SOURCE_LABELS[citation.source] ?? citation.source);
    ...
    if (citation.cell_type) parts.push(citation.cell_type);
  }
  return parts;
}
  ...
  return (
    <li className="text-xs text-slate-700">
      <span className="flex flex-wrap items-center gap-x-2">
        {citation.filename ? <span className="font-medium">{citation.filename}</span> : null}
        {locator ? <span className="text-slate-500">{locator}</span> : null}
        {citation.snippet ? (
          <button
            type="button"
